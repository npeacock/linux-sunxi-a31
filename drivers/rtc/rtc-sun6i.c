...
#define SUN6I_LOSC_CTRL_REG               	(0x0)

#define SUN6I_RTC_DATE_REG                	(0x0010)
#define SUN6I_RTC_TIME_REG                	(0x0014)

#define SUN6I_RTC_ALARM_COUNTER_REG   		(0x0020)
#define SUN6I_RTC_ALARM_CURRENT_REG   		(0x0024)
#define SUN6I_ALARM_EN_REG                	(0x0028)
#define SUN6I_ALARM_INT_CTRL_REG          	(0x002c)
#define SUN6I_ALARM_INT_STATUS_REG        	(0x0030)
...
static int sun6i_rtc_gettime(struct device *dev, struct rtc_time *rtc_tm)
{
	...
	void __iomem *base = sun6i_rtc_base;
...
static int sun6i_rtc_settime(struct device *dev, struct rtc_time *tm)
{
	void __iomem *base = sun6i_rtc_base;
	unsigned int date_tmp = 0;
	unsigned int time_tmp = 0;
	...
	*/
	...
	writel(date_tmp, base + SUN6I_RTC_DATE_REG);
	timeout = 0xffff;
	while((readl(base + SUN6I_LOSC_CTRL_REG)&(RTC_YYMMDD_ACCESS))&&(--timeout))
	if (timeout == 0) {
...
static int sun6i_rtc_setalarm(struct device *dev, struct rtc_wkalrm *alrm)
{
...
static int __devexit sun6i_rtc_remove(struct platform_device *pdev)
{
...
static int __devinit sun6i_rtc_probe(struct platform_device *pdev)
{
	...
	*/
	...
	writel(0x00000000, sun6i_rtc_base + SUN6I_RTC_ALARM_COUNTER_REG);//0x0020
	/*clean the alarm current value*/
	writel(0x00000000, sun6i_rtc_base + SUN6I_RTC_ALARM_CURRENT_REG);//0x0024
	/*disable the alarm0 when init*/
    writel(0x00000000, sun6i_rtc_base + SUN6I_ALARM_EN_REG);//0x0028
...
struct platform_device sun6i_device_rtc = {
	.name		    = "sun6i-rtc",
	.id		        = -1,
	.num_resources	= ARRAY_SIZE(sun6i_rtc_resource),
	.resource	    = sun6i_rtc_resource,
};

static struct platform_driver sun6i_rtc_driver = {
	.probe		= sun6i_rtc_probe,
	.remove		= __devexit_p(sun6i_rtc_remove),
	.driver		= {
		.name	= "sun6i-rtc",
		.owner	= THIS_MODULE,
	},
...
static int __init sun6i_rtc_init(void)
{
	platform_device_register(&sun6i_device_rtc);
	RTC_DBG("sun6i RTC version 0.1 \n");
	return platform_driver_register(&sun6i_rtc_driver);
}

static void __exit sun6i_rtc_exit(void)
{
	platform_driver_unregister(&sun6i_rtc_driver);
}
