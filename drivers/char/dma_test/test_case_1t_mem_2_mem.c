...
u32 __cb_qd_1t_mem_2_mem(dm_hdl_t dma_hdl, void *parg, enum dma_cb_cause_e cause)
{
	...
	case DMA_CB_OK:
		...
		if(ucur_cnt < uloop_cnt) {
			...
			//ucur_saddr = g_src_addr + atomic_read(&g_acur_cnt) * DTC_1T_ONE_LEN;
...
/**
 * __waitdone_1t_mem_2_mem - wait dma transfer function for case DTC_1T_MEM_2_MEM
 *
 * Returns 0 if sucess, the err line number if failed.
 */
...
u32 __dtc_case_enq_aftdone(void)
{
	...
	while(i++ < 30) {
		...
		pr_info("%s: i %d\n", __func__, i);
		ucur_saddr = g_src_addr + 0 * DTC_1T_ONE_LEN;
		ucur_daddr = g_dst_addr + 0 * DTC_1T_ONE_LEN;
		if(0 != sw_dma_enqueue(dma_hdl, ucur_saddr, ucur_daddr, DTC_1T_ONE_LEN, ENQUE_PHASE_NORMAL)) {
			uret = __LINE__;
			goto end;
		}
