...
static int sun6i_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		play_dma_config.dst_drq_type = DRQDST_SPDIFTX;
		...
		capture_dma_config.src_drq_type = DRQSRC_SPDIFRX;
...
static int __devinit sun6i_spdif_pcm_probe(struct platform_device *pdev)
{
	return snd_soc_register_platform(&pdev->dev, &sun6i_soc_platform);
}

static int __devexit sun6i_spdif_pcm_remove(struct platform_device *pdev)
{
...
/*data relating*/
static struct platform_device sun6i_spdif_pcm_device = {
	.name = "sun6i-spdif-pcm-audio",
};

/*method relating*/
static struct platform_driver sun6i_spdif_pcm_driver = {
	.probe = sun6i_spdif_pcm_probe,
	.remove = __devexit_p(sun6i_spdif_pcm_remove),
	.driver = {
		.name = "sun6i-spdif-pcm-audio",
		.owner = THIS_MODULE,
	},
...
static int __init sun6i_soc_platform_spdif_init(void)
{
	int err = 0;
	if((err = platform_device_register(&sun6i_spdif_pcm_device)) < 0)
		return err;

	if ((err = platform_driver_register(&sun6i_spdif_pcm_driver)) < 0)
		return err;
	return 0;
}
module_init(sun6i_soc_platform_spdif_init);

static void __exit sun6i_soc_platform_spdif_exit(void)
{
	return platform_driver_unregister(&sun6i_spdif_pcm_driver);
}
module_exit(sun6i_soc_platform_spdif_exit);

MODULE_AUTHOR("All winner");
MODULE_DESCRIPTION("SUN6I SPDIF DMA module");
MODULE_LICENSE("GPL");
